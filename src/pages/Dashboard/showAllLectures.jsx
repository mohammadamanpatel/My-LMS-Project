...
function DisplayLectures() {
  ...
  // console.log("state.data.lectures[currentVideo].lecture.secure_url",state.data.lectures[currentVideo].lecture.secure_url);
  useEffect(() => {
    if (!state) {
      navigate("/courses");
    }
  ...
  // console.log("lectures[currentVideo].lecture.secure_url",lectures[0].lecture.secure_url)
          ...
          <div className="flex justify-center gap-10 w-full">
            ...
            <ul className="w-[28rem] p-2 rounded-lg shadow-[0_0_10px_black] space-y-5">
              <li className="font-semibold text-xl text-yellow-500 flex items-center justify-between">
                ...
                {role === "ADMIN" && (
                  <button onClick={() => navigate("/course/AddLectures", { state: { ...state } })} className="btn btn-active btn-primary">
                    Add new lecture
                  </button>
                )}
              </li>
              {lectures.map((lecture, idx) => {
                return (
                  <li className="space-y-2" key={lecture._id}>
                    ...
                    {role === "ADMIN" && (
                      <button onClick={() => onLectureDelete(state.data._id, lecture?._id)} className="btn btn-outline btn-warning">
                        Delete lecture
                      </button>
                    )}
                  </li>
