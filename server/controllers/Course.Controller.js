...
const getAllCourses = async function (req, res) {
    try {
        const getAllCoursesDetails = await CourseModel.find({}).select('+lectures');
        return res.status(200).json({
            success: true,
            message: "All Courses details are retrived",
            courses: getAllCoursesDetails
        })
    }
    catch (error) {
        console.log(error);
...
const getCourseById = async function (req, res) {
    try {
        ...
        const getCourseDetailsById = await CourseModel.findById(CourseId);
        return res.status(200).json({
            success: true,
